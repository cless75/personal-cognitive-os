...
5. **Заведи проект** — `/new-project`, затем `/start-session` → `/checkpoint-session` (по ходу) → `/close-session` для ведения работы.
...
| `/checkpoint-session` | Чекпоинт по ходу: решения, сверка с целью, вердикт курса, промпт на продолжение |
